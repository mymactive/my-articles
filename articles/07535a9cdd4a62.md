...
topics: [react, typescript, vitest, test, frontend]
published: false
---
...
実際、上記の表の行をつなげると「タイトルが外れている状態から付けると、タイトルは付いている」というテストケースを得ることができます
...
有限オートマトンを見ると、イベントは`付ける`と`外す`の２種類です
...
この状態でテストを実行すると失敗します
もちろん`strip`というメソッドが`export`されていないからですね
早速実装しましょう
...
これでテストがパスします
`strip`もテストを通すためだけのひどいコードですが、実装は後回しにしてテストを完成させましょう
...
というのも`useTitle`の型エラーを先に解消したいと考えたからです
